...
class MailTest extends Unittest_TestCase {
	...
	{
		return array(array('Mail', array()), array('PEAR_Mail', array()), 
			array('PEAR_Sendmail', array()), array('PEAR_SMTP', array()), 
			array('PHPMailer_Mail', array()), array('PHPMailer_Qmail', array()), 
			array('PHPMailer_Sendmail', array()), array('PHPMailer_SMTP', array()), 
			array('Mock', array()));
	}
	...
	{
		// ascii subect
		$this->assertTrue(Mail_Sender::factory($name, $options)->subject('Hello Foo')
			->from($name)
			->body('test')
			->send(MailTest::RECEIVER));
		...
		// empty subject
		$this->assertTrue(Mail_Sender::factory($name, $options)->subject('')
			->from($name)
			->body('test')
			->send(MailTest::RECEIVER));
	}
